...
func main() {
	...
	flag.DurationVar(&observeTimeout, "observe-timeout", 10*time.Second,
		"Upper bound on one observation round; each observer gets this long.")
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "8932b32c.reidops.com",
		// Uncached: no informer on every Secret, and no hard dependency on the
		// DNSEndpoint CRD at startup.
	...
	if err := (&controller.PublicAddressReconciler{
		Client:    mgr.GetClient(),
		Scheme:    mgr.GetScheme(),
		Recorder:  mgr.GetEventRecorder("external-ddns"),
		Observers: observer.Factory{Secrets: mgr.GetAPIReader(), Timeout: observeTimeout},
		Writer:    &dnsendpoint.Writer{Client: mgr.GetClient(), Scheme: mgr.GetScheme()},
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "publicaddress")
		os.Exit(1)
	}
